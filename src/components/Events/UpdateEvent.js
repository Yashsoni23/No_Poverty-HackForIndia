...
export default function UpdateEvents() {
  ...
  function update(e) {
    ...
    axios
      .put(`https://no-poverty.adaptable.app/events/update/${id}`, data)
      .then(() => {
        Swal.fire({
          title: "Success!",
    ...
    setTimeout(() => navigate(`/Events/VIewEvent/${id}`), 2500);
  }
    ...
    Swal.fire({
      ...
      .then((result) => {
        if (result.isConfirmed) {
          axios.delete("https://no-poverty.adaptable.app/events/delete/" + id);
          Swal.fire({
            title: "Success!",
